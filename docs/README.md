...
Select "Custom installation" during the setup.
...
After installing Python, restart VS Code to ensure it recognizes the Python
installation.
...
Open your terminal and navigate to the Linear Regression project folder.
Type each command line below, pressing Enter after each one to install the library.
Repeat step 2 until all libraries are installed.
USER INTERFACE
UI ELEMENTS IN PREDICTEASE
Main Window (root):
...
Click Open File Explorer in the initial interface(figure).
The select File dialog window opens.
Navigate to the folder you need to import the file button.
...
Select one of the options: “Replace with a number”, “Replace with mean”, replace with
...
constant value.
Table 1 : Options in the Handle missing Data drop-down list.
